/*
 ...
 Current defined devices are:
	...
	version 2.0 increased payload size; implemented node uptime; standard device type convention; error handling.
        version 2.1 add low power mode and RX cycle; debugging improvements;
*/
...
#define ENCRYPTKEY "sampleEncryptKey" 			// 16-char encryption key; same as on Gateway!
...
//
// sleep/low power
long     SLEEP_INTERVAL = 455;
long     TX_INTERVAL = 15000;
long     RX_CYCLE = 45;
long 	TXinterval = 10;				// periodic transmission interval in seconds
...
//	VARIABLES
//
long	lastPeriod = -TXinterval;				// timestamp last transmission
...
DHT dht(DHTPIN, DHTTYPE, 2);			// initialise temp/humidity sensor for 3.3 Volt arduino
RFM69 radio;
...
void loop() {
  ...
  if (curState == LOW && lastState == HIGH) {		// button pressed ?
    ...
    if (ackButton && !msgBlock) send40 = true;	// set button message flag
...
void txRadio()						// Transmits the 'mes'-struct to the gateway
{
  ...
  else DEBUGln("No connection...");
}	// end txRadio


void powerDown(long ms) {
  radio.sleep();
  ...
  while (milliseconds > 0 ) {
    if (milliseconds >= 8000) {
      ...
      sleep(SLEEP_250MS);
      milliseconds -= 250;
    } else if (milliseconds >= 125)     {
      sleep(SLEEP_120MS);
      milliseconds -= 120;
    } else if (milliseconds >= 64)      {
      sleep(SLEEP_60MS);
      milliseconds -= 60;
    } else if (milliseconds >= 32)      {
      sleep(SLEEP_30MS);
      milliseconds -= 30;
  ...
  // FIX FOR MILLIS STOPPING - WE MANUALLY INCREMENT IT BY DURATION OF SLEEP
